...
const AdminSGSDashboard = () => {
  ...
  const loadData = async () => {
    ...
    const [risksRes, incidentsRes, actionsRes, staffRes, surveysRes, briefingsRes, termsRes, veiculosRes, condutoresRes, checklistsRes, bookingsRes, equipmentRes, proceduresRes] = await Promise.all([
      supabase.from("sgs_risks").select("status, risk_level"),
      supabase.from("sgs_incidents").select("type, description, created_at, severity, status").order("created_at", { ascending: false }),
      supabase.from("sgs_corrective_actions").select("status").in("status", ["pendente", "em_andamento"]),
      supabase.from("sgs_staff_trainings").select("status").eq("status", "vencendo"),
      ...
      supabase.from("sgs_veiculos").select("seguro_validade, placa, marca, modelo"),
      supabase.from("sgs_condutores").select("id", { count: "exact", head: true }),
    ...
    setStats({
      ...
      condutores: condutoresRes.count || 0,
