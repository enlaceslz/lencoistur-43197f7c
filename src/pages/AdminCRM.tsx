...
import {
  Search, Mail, Eye, Download, Loader2, Users,
  DollarSign, MapPin, Smartphone, RefreshCw, Plus,
  Pencil, Trash2, X, Save, UserPlus, Baby, FileText,
  Tag as TagIcon, Target, CheckCircle2
} from "lucide-react";
import CustomerWideView from "@/components/CustomerWideView";
...
const AdminCRMContent = () => {
  ...
  return isWideViewNewWindow ? (
    ...
    <AdminLayout title="Gestão de Clientes & CRM">
      ...
      {/* Wide View Modal - Visualização Ampla na mesma tela */}
      <Dialog open={isWideViewOpen} onOpenChange={setIsWideViewOpen}>
        <DialogContent className="max-w-6xl w-[95vw] h-[90vh] overflow-y-auto p-0 rounded-3xl border-none shadow-2xl">
            ...
            <div className="bg-white border-b border-slate-100 p-6 flex items-center justify-between sticky top-0 z-10">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  onClick={() => {
                    const url = `${window.location.origin}${window.location.pathname}?wide_view_id=${selectedCustomer?.id}`;
                ...
                <Button 
                  ...
                  size="sm" 
                  className="rounded-xl font-bold h-10"
                  onClick={() => setIsWideViewOpen(false)}
                >
                  Fechar
                </Button>
              </div>
